...
FACTORY_DEFAULTS = {
    ...
    "paragraph_spacing_px": 0,  # 0=与行间距一致，不额外加段间距
    "toc_font_size": 28,  # 目录字号，与正文一致时可与 font_size 设为相同
    "toc_indent_px": 24,  # 目录二级标题相对一级的缩进像素
    "font_name": "",
    "preview_zoom": 440,
    "generate_toc": True,
...
def get_official_toc_mapping(book):
    """返回 mapping[filename] = [(anchor, title, level), ...]，level 1=一级标题，2=二级标题。"""
    mapping = {}

    def add_entry(href, title, level=1):
        if '#' in href:
        ...
        if filename not in mapping:
            mapping[filename] = []
        mapping[filename].append((anchor, title, level))

    def process_toc_item(item, level=1):
        if isinstance(item, tuple):
            if len(item) > 1 and isinstance(item[1], list):
                # 只处理子项，子项为二级标题（不添加 parent，保持与原有条目数量一致）
                for sub in item[1]:
                    process_toc_item(sub, level + 1)
        elif isinstance(item, epub.Link):
            add_entry(item.href, item.title, level)
            ...
            try:
                ...
                if nav_element:
                    for link in nav_element.find_all('a', href=True):
                        add_entry(link['href'], link.get_text().strip(), 1)
            except:
                pass
    return mapping


def infer_toc_level_from_title(title):
    """根据标题文本推断 TOC 层级：壹贰叁/序/导读等为一级，01 02 03 为二级。"""
    if not title or not isinstance(title, str):
        return None
    t = title.strip()
    if not t:
        return None
    # 两位数字+空格开头 -> 二级（如 01 一则…、02 集体…）
    if re.match(r'^\d{2}\s', t):
        return 2
    # 中文章节数或常见一级标题开头 -> 一级（壹贰叁、序、导读、前言等）
    if re.match(r'^[壹贰叁肆伍陆柒捌玖拾零一二三四五六七八九十百千序导读前言查]', t):
        return 1
    return None
...
class CoverExportDialog(ctk.CTkToplevel):
    def __init__(self, parent, callback):
        ...
        ctk.CTkLabel(self,
                     text=_("Crop to Fill: Fills the screen, cuts off edges if ratio differs.\nFit: Keeps entire image, adds white bars.\nStretch: Forces image to size, may look squashed."),
                     font=("Arial", 12), text_color="gray").pack(pady=10)
        ctk.CTkButton(self, text=_("Export"), command=self.confirm).pack(pady=20)
    ...
    def confirm(self):
        try:
            ...
            messagebox.showerror(_("Error"), _("Width and Height must be numbers."))
            return
...
class ChapterSelectionDialog(ctk.CTkToplevel):
    def __init__(self, parent, chapters_list, initial_selection, callback):
        ...
        self.title(_("Select Chapters for TOC"))
        self.geometry("500x600")
        self.transient(parent)
        self.grab_set()

        self.lbl_info = ctk.CTkLabel(self,
                                     text=_("Uncheck chapters to hide them from the TOC and Progress Bar.\n(They will still be readable in the book)"),
                                     wraplength=450)
        self.lbl_info.pack(pady=10)

        self.btn_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.btn_frame.pack(fill="x", padx=20)
        ctk.CTkButton(self.btn_frame, text=_("Select All"), width=100, command=self.select_all).pack(side="left", padx=5)
        ctk.CTkButton(self.btn_frame, text=_("Select None"), width=100, command=self.select_none).pack(side="left", padx=5)
        ...
        self.btn_confirm = ctk.CTkButton(self, text=_("Confirm Selection"), command=self.confirm)
    ...
    def confirm(self):
        ...
        if not selected_indices:
            if not messagebox.askyesno(_("Warning"),
                                       _("No chapters selected. The TOC will be empty. Continue?")):
                return
...
class EpubProcessor:
    ...
    def _split_html_by_toc(self, soup, toc_entries):
        chunks = []
        # 兼容 (anchor, title) 或 (anchor, title, level)
        def get_level(entry):
            return entry[2] if len(entry) > 2 else 1
        if len(toc_entries) == 1 and not toc_entries[0][0]:
            return [{'title': toc_entries[0][1], 'soup': soup, 'level': get_level(toc_entries[0])}]
        split_points = []
        for entry in toc_entries:
            anchor, title = entry[0], entry[1]
            level = get_level(entry)
            target = None
            if anchor: target = soup.find(id=anchor)
            if target or not anchor: split_points.append({'node': target, 'title': title, 'level': level})
        if not split_points: return [{'title': toc_entries[0][1], 'soup': soup, 'level': get_level(toc_entries[0])}]
        ...
        for child in body_children:
            ...
            if current_idx + 1 < len(split_points):
                ...
                if next_node and (child == next_node or is_nested_target):
                    chunks.append({'title': split_points[current_idx]['title'], 'soup': current_soup, 'level': split_points[current_idx]['level']})
                    current_idx += 1
                    current_soup = BeautifulSoup("<body></body>", 'html.parser')
    ...
    def parse_book_structure(self, input_path):
        ...
        for item in items:
            ...
            if toc_entries and len(toc_entries) > 1:
                ...
                for chunk in split_chapters:
                    self.raw_chapters.append(
                        {'title': chunk['title'], 'soup': chunk['soup'], 'has_image': bool(chunk['soup'].find('img')),
                         'filename': item_filename, 'level': chunk.get('level', 1)})
            else:
                ...
                self.raw_chapters.append(
                    {'title': chapter_title, 'soup': soup, 'has_image': has_image, 'filename': item_filename, 'level': 1})
        return True
    ...
    def render_chapters(self, selected_indices, font_path, font_size, margin, line_height, font_weight, bottom_padding,
        ...
        paragraph_indent_px = max(0, int(self.layout_settings.get("paragraph_indent_px", 40)))
        paragraph_spacing_px = max(0, int(self.layout_settings.get("paragraph_spacing_px", 0)))
        ...
        custom_css = f"""
                    <style>
                        ...
                        /* 2. BODY CONTAINER CONTROLS（左右边距一致，两端对齐） */
                        body {{
                        ...
                        /* 3. OVERRIDE CALIBRE/GENERIC TAGS（继承两端对齐与 text-justify，保证右侧齐平） */
                        p, div, li, dd, dt, span, blockquote {{
                            ...
                            margin-top: {paragraph_spacing_px}px !important;
                            margin-bottom: {paragraph_spacing_px}px !important;
                            max-width: 100% !important;
                        }}

                        /* 首行缩进仅用于段落与列表项，不用于 div/span/blockquote（避免标题被缩进） */
                        p, li, dd, dt {{
                            text-indent: {paragraph_indent_px}px;
                        }}
                        ...
                        h1 {{ font-size: 1.35em !important; margin-top: 0.8em; margin-bottom: 0.2em !important; }}
                        h2 {{ font-size: 1.25em !important; margin-top: 0.7em; margin-bottom: 0.2em !important; }}
                        h3 {{ font-size: 1.15em !important; margin-top: 0.6em; margin-bottom: 0.2em !important; }}
                        ...
                        /* 标题类（含 class 中含 title/heading/chapter 的 p 或 div）不首行缩进 */
                        p[class*="title"], p[class*="heading"], p[class*="chapter"],
                        div[class*="title"], div[class*="heading"], div[class*="chapter"] {{
                            text-indent: 0 !important;
                        }}

                        /* 标题正下方的一行（如作者、副标题）居中、不缩进、无上下空隙 */
                        ...
                        p[class*="author"], p[class*="byline"], .author, .byline {{
                            text-align: center !important;
                            text-indent: 0 !important;
                            margin-top: 0 !important;
                            margin-bottom: 0 !important;
                        }}
                        /* 紧接在“标题类”段落后的段落（如作者行）也居中且无上下空隙 */
                        p[class*="title"] + p, p[class*="heading"] + p, div[class*="title"] + p, div[class*="heading"] + p {{
                            text-align: center !important;
                            text-indent: 0 !important;
                            margin-top: 0 !important;
                            margin-bottom: 0 !important;
        ...
        for idx, chapter in enumerate(self.raw_chapters):
            ...
            if idx in selected_set:
                ...
                struct_level = chapter.get('level', 1)
                inferred = infer_toc_level_from_title(chapter['title'])
                level = inferred if inferred is not None else struct_level
                final_toc_titles.append((chapter['title'], level))
        ...
        if add_toc and final_toc_titles:
            toc_main_size = self.layout_settings.get("toc_font_size", self.font_size) or self.font_size
            toc_main_size = max(12, int(toc_main_size))
            toc_header_size = int(toc_main_size * 1.2)
            toc_row_height = int(toc_main_size * self.line_height * 1.2)
        ...
        self.toc_data_final = []
        for i, item in enumerate(final_toc_titles):
            title, level = (item[0], item[1]) if isinstance(item, (tuple, list)) and len(item) >= 2 else (item, 1)
            ...
            self.toc_data_final.append((title, display_page, level))
    ...
    def _render_toc_pages(self, toc_entries, row_height, font_size, header_size):
        ...
        # 二级标题相对一级的缩进像素（可在 layout_settings 中配置 toc_indent_px）
        toc_indent_px = self.layout_settings.get("toc_indent_px", 24)
        ...
        for i in range(0, len(toc_entries), limit):
            ...
            for entry in chunk:
                # 兼容 (title, pg_num) 或 (title, pg_num, level)
                if len(entry) >= 3:
                    title, pg_num, level = entry[0], entry[1], entry[2]
                else:
                    title, pg_num, level = entry[0], entry[1], 1
                indent = (level - 1) * toc_indent_px
                x_title = left_margin + indent
                pg_str = str(pg_num)
                pg_w = font_main.getlength(pg_str)
                max_title_w = self.screen_width - left_margin - right_margin - pg_w - column_gap - indent
                display_title = title
                if font_main.getlength(display_title) > max_title_w:
                    ...
                    display_title += "..."
                draw.text((x_title, y), display_title, font=font_main, fill=0)
                title_end_x = x_title + font_main.getlength(display_title) + 5
    ...
    def _get_page_text_elements(self, global_page_index):
        ...
        if global_page_index < num_toc:
            ...
            for item in reversed(self.toc_data_final):
                title, start_pg = item[0], item[1]
    ...
    def render_page(self, global_page_index):
        ...
        if num_toc > 0 and toc_start <= global_page_index < (toc_start + num_toc):
            ...
            # 抗锯齿：部分 PyMuPDF 安装无 Tools，仅在有该 API 时设置
            try:
                tools = getattr(fitz, "Tools", None)
                if tools is not None and hasattr(tools, "set_aa_level"):
                    level = 8 if self.layout_settings.get("render_antialias", True) else 0
    ...
    def save_xtc(self, out_name, progress_callback=None):
        ...
        for i, item in enumerate(self.toc_data_final):
            title, start_pg_from_renderer = item[0], item[1]
