...
from locale_zh import (
    t as trans_zh,
    sort_value_to_display,
    sort_display_to_value,
    view_value_to_display,
    view_display_to_value,
    get_sort_options,
    get_view_options,
    get_align_options,
    align_value_to_display,
    align_display_to_value,
    get_orientation_options,
    orientation_value_to_display,
    orientation_display_to_value,
    get_pos_options,
    pos_value_to_display,
    pos_display_to_value,
    get_pos_progress_options,
    pos_progress_value_to_display,
    pos_progress_display_to_value,
)
...
class SettingsDialog(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.title(trans_zh("Xlibre Manager Settings"))
        ...
        ctk.CTkLabel(self, text=trans_zh("Manager Preferences"), font=("Arial", 20, "bold")).pack(pady=20)

        # Library Path
        ctk.CTkLabel(self, text=trans_zh("Library Location:"), anchor="w").pack(fill="x", padx=30)
        ...
        # Device IP
        ctk.CTkLabel(self, text=trans_zh("Default Device IP (Reader):"), anchor="w").pack(fill="x", padx=30)
        self.entry_ip = ctk.CTkEntry(self)
        self.entry_ip.insert(0, self.config["device_ip"])
        self.entry_ip.pack(fill="x", padx=30, pady=(0, 15))

        # Default Sorting
        ctk.CTkLabel(self, text=trans_zh("Startup Sorting:"), anchor="w").pack(fill="x", padx=30)
        sort_opts = get_sort_options()
        self.sort_values = [v for _, v in sort_opts]
        self.sort_displays = [d for d, _ in sort_opts]
        self.sort_opt = ctk.CTkOptionMenu(self, values=self.sort_displays)
        self.sort_opt.set(sort_value_to_display(self.config["default_sort"]))
        self.sort_opt.pack(fill="x", padx=30, pady=(0, 15))

        # View Mode
        ctk.CTkLabel(self, text=trans_zh("Startup View:"), anchor="w").pack(fill="x", padx=30)
        view_opts = get_view_options()
        self.view_displays = [d for d, _ in view_opts]
        self.view_opt = ctk.CTkOptionMenu(self, values=self.view_displays)
        self.view_opt.set(view_value_to_display(self.config["view_mode"]))
        ...
        ctk.CTkSwitch(self, text=trans_zh("Embed custom cover into original .EPUB file"), variable=self.var_update_epub).pack(
            fill="x", padx=30, pady=(10, 15))

        ctk.CTkButton(self, text=trans_zh("Save and Restart App"), fg_color="#E67E22", hover_color="#D35400",
                      command=self.save_and_restart).pack(pady=30)

        ctk.CTkLabel(self, text=trans_zh("*App will restart to apply all changes"),
    ...
    def save_and_restart(self):
        ...
        if os.path.normpath(new_base) != os.path.normpath(old_base):
            if messagebox.askyesno(trans_zh("Migrate Library"), trans_zh("Move library to:\n{}?").format(new_base)):
                try:
                    ...
                    messagebox.showerror(trans_zh("Migration Error"), str(e))
                    return
        ...
        try:
            ...
            if hasattr(self.parent, 'db'):
                messagebox.showinfo(
                    trans_zh("Settings Saved"),
                    trans_zh("Your settings have been updated.\n\nPlease manually restart Xlibre to apply all changes.")
                )
    ...
    def _final_save(self, base_path):
        new_config = {
            ...
            "default_sort": sort_display_to_value(self.sort_opt.get()),
            "view_mode": view_display_to_value(self.view_opt.get()),
...
class IntegratedEditor(ctk.CTkToplevel):
    def __init__(self, book_data, db_instance, refresh_callback, parent):
        ...
        # Window Setup
        self.title(trans_zh("Convert: {}").format(book_data[1]))
    ...
    def import_custom_font(self):
        """Import font files (.ttf, .otf) or .zip, or a folder of fonts."""
        # 1) 选择文件：字体或 ZIP
        filetypes = [
            (trans_zh("Font files") + " (.ttf, .otf)", "*.ttf *.otf"),
            ("ZIP " + trans_zh("archives"), "*.zip"),
            (trans_zh("All files"), "*.*"),
        ]
        paths = filedialog.askopenfilenames(
            title=trans_zh("Select Font File(s) or ZIP"),
            filetypes=filetypes,
        )
        if paths:
            self._import_font_paths(list(paths))
            return
        # 2) 选择文件夹（原有逻辑）
        src_folder = filedialog.askdirectory(title=trans_zh("Select Font Family Folder"))
        if not src_folder:
            return
        family_name = os.path.basename(src_folder)
        dest_folder = os.path.join(FONTS_DIR, family_name)
        if os.path.exists(dest_folder):
            if not messagebox.askyesno(trans_zh("Overwrite"), trans_zh("Font family '{}' exists. Overwrite?").format(family_name)):
                return
            shutil.rmtree(dest_folder)
        try:
            shutil.copytree(src_folder, dest_folder)
            messagebox.showinfo(trans_zh("Success"), trans_zh("Imported: {}").format(family_name))
            self.refresh_fonts_list()
        except Exception as e:
            messagebox.showerror(trans_zh("Error"), str(e))

    def _import_font_paths(self, paths):
        """Import one or more font files or ZIP archives into FONTS_DIR."""
        imported = []
        for path in paths:
            ...
            try:
                if low.endswith(".zip"):
                    ...
                    dest_dir = os.path.join(FONTS_DIR, stem)
                    if os.path.exists(dest_dir):
                        if not messagebox.askyesno(trans_zh("Overwrite"), trans_zh("Font family '{}' exists. Overwrite?").format(stem)):
                            continue
                ...
                messagebox.showerror(trans_zh("Error"), f"{name}: {e}")
        if imported:
            msg = ", ".join(imported[:5]) + ("..." if len(imported) > 5 else "")
            messagebox.showinfo(trans_zh("Success"), trans_zh("Imported: {}").format(msg))
            self.refresh_fonts_list()
    ...
    def _create_slider(self, parent, label_attr, text, slider_attr, min_v, max_v, is_float=False, width=None,
        ...
        # Register attributes（供预设加载时同步输入框与标签）
        setattr(self, f"fmt_{slider_attr}", get_label_text)
        setattr(self, slider_attr, sld)
        setattr(self, slider_attr + '_entry', entry)
        setattr(self, slider_attr + '_is_float', is_float)
        setattr(self, slider_attr + '_label_attr', label_attr)

        return sld

    def _sync_slider_ui(self, slider_attr):
        """加载预设后同步该滑块对应的标签和输入框显示。"""
        sld = getattr(self, slider_attr, None)
        if not sld or not hasattr(sld, 'get'):
            return
        entry = getattr(self, slider_attr + '_entry', None)
        label_attr = getattr(self, slider_attr + '_label_attr', None)
        fmt_fn = getattr(self, f"fmt_{slider_attr}", None)
        if not entry or not label_attr or not fmt_fn:
            return
        val = sld.get()
        is_float = getattr(self, slider_attr + '_is_float', False)
        fmt_val = round(val, 1) if is_float else int(val)
        entry.delete(0, "end")
        entry.insert(0, str(fmt_val))
    ...
    def _build_toolbar(self):
        ...
        ctk.CTkLabel(logo_f, text=trans_zh("EDITOR"), font=("Arial", 20, "bold"), text_color="#3498DB").pack(anchor="w")
        ...
        self.btn_font_import = self._create_icon_btn(tb, trans_zh("Aa Import Font"), "#34495E", self.import_custom_font)
        # Save XTC -> Updates DB
        self.btn_export = self._create_icon_btn(tb, trans_zh("💾 Save to Library"), "#2ECC71", self.save_to_library, "disabled")
        self.btn_cover = self._create_icon_btn(tb, trans_zh("🖼 Export Cover"), "#8E44AD", self.open_cover_export, "disabled")

        right_f = ctk.CTkFrame(tb, fg_color="transparent")
        right_f.pack(side="right", padx=20, fill="y")
        self.progress_label = ctk.CTkLabel(right_f, text=trans_zh("Loading..."), font=("Arial", 12), text_color="gray",
                                           anchor="e")
    ...
    def save_current_as_default(self):
        ...
        try:
            ...
            messagebox.showinfo(trans_zh("Saved"), trans_zh("Current settings saved as default."))
        except Exception as e:
            messagebox.showerror(trans_zh("Error"), trans_zh("Could not save defaults: {}").format(e))
    ...
    def _build_sidebar(self):
        ...
        # PRESETS
        c_pre = converter.SettingsCard(self.sidebar, trans_zh("PRESETS"))
        row_pre = ctk.CTkFrame(c_pre.content, fg_color="transparent")
        row_pre.pack(fill="x")
        self.preset_var = ctk.StringVar(value=trans_zh("Select Preset..."))
        ...
        # RENDER ENGINE
        c_ren = converter.SettingsCard(self.sidebar, trans_zh("RENDER ENGINE"))
        r_depth = ctk.CTkFrame(c_ren.content, fg_color="transparent")
        r_depth.pack(fill="x", pady=2)
        ctk.CTkLabel(r_depth, text=trans_zh("Target Format:"), width=130, anchor="w").pack(side="left")
        ...
        ctk.CTkButton(c_pre.content, text=trans_zh("Set Current as Default"), command=self.save_current_as_default,
        ...
        ctk.CTkLabel(r_mode, text=trans_zh("Conversion:"), width=130, anchor="w").pack(side="left")
        ...
        self.var_render_antialias = ctk.BooleanVar(value=self.startup_settings.get("render_antialias", True))
        ctk.CTkCheckBox(c_ren.content, text=trans_zh("Antialiasing"), variable=self.var_render_antialias,
                        command=self.schedule_update).pack(anchor="w", pady=5, padx=5)
        ...
        self.sld_white = self._create_slider(self.frm_dither, "lbl_white_clip", trans_zh("White Clip"), "slider_white_clip", 150,
                                             255)
        self.sld_contrast = self._create_slider(self.frm_dither, "lbl_contrast", trans_zh("Contrast"), "slider_contrast", 0.5,
                                                2.0, is_float=True)
        self.frm_thresh = ctk.CTkFrame(self.frm_render_dynamic, fg_color="transparent")
        self.sld_thresh = self._create_slider(self.frm_thresh, "lbl_threshold", trans_zh("Threshold"), "slider_text_threshold",
                                              50, 200)
        self.sld_blur = self._create_slider(self.frm_thresh, "lbl_blur", trans_zh("Definition"), "slider_text_blur", 0.0, 3.0,
                                            is_float=True)

        # TYPOGRAPHY
        c_type = converter.SettingsCard(self.sidebar, trans_zh("TYPOGRAPHY"))
        r_font = ctk.CTkFrame(c_type.content, fg_color="transparent")
        r_font.pack(fill="x", pady=2)
        ctk.CTkLabel(r_font, text=trans_zh("Font Family:"), width=130, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(r_align, text=trans_zh("Alignment:"), width=130, anchor="w").pack(side="left")
        self.align_dropdown = ctk.CTkOptionMenu(
            r_align,
            values=[disp for disp, _ in get_align_options()],
        ...
        ctk.CTkCheckBox(c_type.content, text=trans_zh("Hyphenate Text"), variable=self.var_hyphenate, command=self.schedule_update).pack(anchor="w", pady=5, padx=5)

        self._create_slider(c_type.content, "lbl_size", trans_zh("Font Size"), "slider_font_size", 12, 48)
        self._create_slider(c_type.content, "lbl_weight", trans_zh("Font Weight"), "slider_font_weight", 100, 900)
        self._create_slider(c_type.content, "lbl_line", trans_zh("Line Height"), "slider_line_height", 1.0, 2.5, is_float=True)
        self._create_slider(c_type.content, "lbl_word_space", trans_zh("Word Spacing"), "slider_word_spacing", 0.0, 2.0,
                            is_float=True)
        self._create_slider(c_type.content, "lbl_paragraph_indent_px", trans_zh("Paragraph Indent (px):"), "slider_paragraph_indent_px", 0, 120)
        self._create_slider(c_type.content, "lbl_paragraph_spacing_px", trans_zh("Paragraph Spacing (px):"), "slider_paragraph_spacing_px", 0, 80)

        # LAYOUT
        c_lay = converter.SettingsCard(self.sidebar, trans_zh("PAGE LAYOUT"))
        r_ori = ctk.CTkFrame(c_lay.content, fg_color="transparent")
        r_ori.pack(fill="x", pady=2)
        ctk.CTkLabel(r_ori, text=trans_zh("Orientation:"), width=130, anchor="w").pack(side="left")
        self.orientation_var = ctk.StringVar(value=orientation_value_to_display("Portrait"))
        ctk.CTkOptionMenu(
            r_ori,
            values=[disp for disp, _ in get_orientation_options()],
        ...
        ctk.CTkCheckBox(r_tog, text=trans_zh("Generate TOC"), variable=self.var_toc, command=self.schedule_update).pack(
            side="left")
        self.var_footnotes = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(r_tog, text=trans_zh("Inline Footnotes"), variable=self.var_footnotes, command=self.schedule_update).pack(
            side="right")
        self._create_slider(c_lay.content, "lbl_margin", trans_zh("Side Margin"), "slider_margin", 0, 100)
        self._create_slider(c_lay.content, "lbl_top_padding", trans_zh("Top Padding"), "slider_top_padding", 0, 150)
        self._create_slider(c_lay.content, "lbl_padding", trans_zh("Bottom Padding"), "slider_bottom_padding", 0, 150)

        # --- TOC POSITION & TOC FONT SIZE ---
        r_toc_pos = ctk.CTkFrame(c_lay.content, fg_color="transparent")
        r_toc_pos.pack(fill="x", pady=2)
        ctk.CTkLabel(r_toc_pos, text=trans_zh("TOC Insert Page:"), width=130, anchor="w").pack(side="left")
        ...
        self._create_slider(c_lay.content, "lbl_toc_font_size", trans_zh("TOC Font Size:"), "slider_toc_font_size", 12, 48)

        # HEADER & FOOTER
        c_hf = converter.SettingsCard(self.sidebar, trans_zh("HEADER & FOOTER"))
        grid_hf = ctk.CTkFrame(c_hf.content, fg_color="transparent")
        grid_hf.pack(fill="x")

        # SPECTRA AI
        c_spectra = converter.SettingsCard(self.sidebar, trans_zh("SPECTRA AI ANNOTATIONS"))
        if not converter.HAS_WORDFREQ or not converter.HAS_OPENAI:
            ctk.CTkLabel(c_spectra.content, text=trans_zh("Missing libraries:\npip install wordfreq openai"),
                         text_color="#C0392B").pack()
            self.var_spectra_enabled = ctk.BooleanVar(value=False)
        else:
            self.var_spectra_enabled = ctk.BooleanVar(value=False)
            ctk.CTkCheckBox(c_spectra.content, text=trans_zh("Show Definitions Overlay"), variable=self.var_spectra_enabled,
                            command=self.schedule_update).pack(anchor="w", pady=5)
            ctk.CTkLabel(c_spectra.content, text=trans_zh("Target Language:"), anchor="w").pack(anchor="w")
            ...
            self.btn_spectra_gen = ctk.CTkButton(c_spectra.content, text=trans_zh("⚡ Analyze & Generate"),
            ...
            self._create_slider(c_spectra.content, "lbl_spectra_thresh", trans_zh("Zipf Difficulty"), "slider_spectra_threshold",
                                1.0, 7.0, is_float=True, trigger_auto_update=False)
            self._create_slider(c_spectra.content, "lbl_spectra_aoa", trans_zh("Min. Age of Acquisition"),
        ...
        # Header/Footer Positioning Helper（界面显示中文，配置存英文）
        def add_elem_row(txt, var_pos_name, var_ord_name):
            r = ctk.CTkFrame(grid_hf, fg_color="transparent")
            r.pack(fill="x", pady=2)
            ctk.CTkLabel(r, text=trans_zh(txt), width=110, anchor="w").pack(side="left")
            var_p = ctk.StringVar(value=pos_value_to_display("Hidden"))
            setattr(self, f"var_{var_pos_name}", var_p)
            ctk.CTkOptionMenu(
                r,
                variable=var_p,
                values=[disp for disp, _ in get_pos_options()],
                width=90,
                height=22,
                command=self.schedule_update,
        ...
        ctk.CTkLabel(row_progress, text=trans_zh("Progress Bar:"), width=130, anchor="w").pack(side="left")
        self.var_pos_progress = ctk.StringVar(value=pos_progress_value_to_display("Footer (Below Text)"))
        ctk.CTkOptionMenu(
            row_progress,
            variable=self.var_pos_progress,
            values=[disp for disp, _ in get_pos_progress_options()],
            command=self.schedule_update,
            height=22,
        ...
        self._create_slider(c_hf.content, "lbl_marker_size", trans_zh("Marker Radius"), "slider_bar_marker_radius", 2, 10)
        self._create_slider(c_hf.content, "lbl_tick_height", trans_zh("Tick Height"), "slider_bar_tick_height", 2, 20)
        self._create_slider(c_hf.content, "lbl_bar_thick", trans_zh("Bar Thickness"), "slider_bar_height", 1, 10)
        ...
        ctk.CTkLabel(r_h_align, text=trans_zh("Header Align:"), width=130, anchor="w").pack(side="left")
        ...
        self._create_slider(f_adv, "lbl_header_size", trans_zh("Header Size"), "slider_header_font_size", 8, 30)
        self._create_slider(f_adv, "lbl_header_margin", trans_zh("Header Y"), "slider_header_margin", 0, 80)

        self._create_divider_horizontal(f_adv)

        r_f_align = ctk.CTkFrame(f_adv, fg_color="transparent");
        r_f_align.pack(fill="x", pady=2)
        ctk.CTkLabel(r_f_align, text=trans_zh("Footer Align:"), width=130, anchor="w").pack(side="left")
        ...
        ctk.CTkOptionMenu(r_f_align, variable=self.var_footer_align, values=["Left", "Center", "Right", "Justify"],
                          command=self.schedule_update, height=22).pack(side="right", fill="x")
        self._create_slider(f_adv, "lbl_footer_size", trans_zh("Footer Size"), "slider_footer_font_size", 8, 30)
        self._create_slider(f_adv, "lbl_footer_margin", trans_zh("Footer Y"), "slider_footer_margin", 0, 80)

        self._create_divider_horizontal(f_adv)

        r_ui_style = ctk.CTkFrame(f_adv, fg_color="transparent")
        r_ui_style.pack(fill="x", pady=2)
        ctk.CTkLabel(r_ui_style, text=trans_zh("UI Font:"), width=130, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(r_sep, text=trans_zh("Separator:"), width=130, anchor="w").pack(side="left")
        ...
        self._create_slider(f_adv, "lbl_ui_margin", trans_zh("Side Margin"), "slider_ui_side_margin", 0, 100)
    ...
    def _build_preview_area(self):
        ...
        self._create_slider(f_zoom, "lbl_preview_zoom", trans_zh("Zoom"), "slider_preview_zoom", 200, 800, width=150,
                            label_width=75)
    ...
    def _on_structure_parsed(self, success):
        if not success:
            messagebox.showerror(trans_zh("Error"), trans_zh("Failed to parse EPUB."))
            return
    ...
    def run_processing(self):
        ...
        self.pending_rerun = False
        self.progress_label.configure(text=trans_zh("Rendering Layout..."))
    ...
    def _done(self, success):
        ...
        self.progress_label.configure(text=trans_zh("Ready"))
        self.progress_bar.set(0)
        ...
        if success and self.processor.total_pages > 0:
            ...
            self.img_label.configure(text=trans_zh("No pages rendered."))
    ...
    def _on_export_complete(self, path):
        ...
        messagebox.showinfo(trans_zh("Success"), trans_zh("Book converted and saved to library."))
    ...
    def run_spectra_analysis(self):
        ...
        if not self.entry_spectra_key.get():
            messagebox.showerror(trans_zh("Missing Key"), trans_zh("Please enter an OpenAI API Key."))
            return

        force_regen = False
        if hasattr(self.processor.annotator, 'master_cache') and self.processor.annotator.master_cache:
            ans = messagebox.askyesnocancel(trans_zh("Regenerate?"), trans_zh("Definitions exist. Force regenerate?"))
            if ans is None: return
            force_regen = ans

        self.is_processing = True
        self.progress_label.configure(text=trans_zh("Scanning Words..."))
        settings = self.gather_current_ui_settings()
    ...
    def show_page(self, idx):
        ...
        if not self.processor.is_ready or not self.processor.page_map:
            self.img_label.configure(image=None, text=trans_zh("Calculating Layout..."))
            return
    ...
    def open_cover_export(self):
        if self.processor.cover_image_obj:
            ...
            messagebox.showinfo(trans_zh("Info"), trans_zh("No cover found."))
    ...
    def _run_cover_export(self, path, w, h, mode):
        try:
            ...
            self.after(0, lambda: messagebox.showinfo(trans_zh("Success"), trans_zh("Cover saved.")))
        except Exception as e:
            self.after(0, lambda: messagebox.showerror(trans_zh("Error"), str(e)))

    def schedule_update(self, _=None):
        self.progress_label.configure(text=trans_zh("Pending changes..."))
    ...
    def refresh_presets_list(self):
        ...
        names.sort()
        self.preset_dropdown.configure(values=names if names else [trans_zh("No Presets")])

    def load_selected_preset(self, choice):
        if choice in ["No Presets", "Select Preset...", trans_zh("No Presets"), trans_zh("Select Preset...")]: return
        path = os.path.join(PRESETS_DIR, f"{choice}.json")
        if os.path.exists(path):
            with open(path, "r") as f: self.apply_settings_dict(json.load(f))

    def save_new_preset(self):
        name = simpledialog.askstring(trans_zh("New Preset"), trans_zh("Name:"))
        if name:
    ...
    def gather_current_ui_settings(self):
        ...
        return {
            ...
            "paragraph_indent_px": int(self.slider_paragraph_indent_px.get()) if hasattr(self, "slider_paragraph_indent_px") else 40,
            "paragraph_spacing_px": int(self.slider_paragraph_spacing_px.get()) if hasattr(self, "slider_paragraph_spacing_px") else 0,
            "toc_font_size": int(self.slider_toc_font_size.get()) if hasattr(self, "slider_toc_font_size") else 28,
            ...
            "orientation": orientation_display_to_value(self.orientation_var.get()),
            "text_align": align_display_to_value(self.align_dropdown.get()),
            ...
            "pos_title": pos_display_to_value(self.var_pos_title.get()),
            "pos_pagenum": pos_display_to_value(self.var_pos_pagenum.get()),
            "pos_chap_page": pos_display_to_value(self.var_pos_chap_page.get()),
            "pos_percent": pos_display_to_value(self.var_pos_percent.get()),
            "pos_progress": pos_progress_display_to_value(self.var_pos_progress.get()),
            ...
            "render_antialias": self.var_render_antialias.get() if hasattr(self, "var_render_antialias") else True,
    ...
    def apply_settings_dict(self, s):
        ...
        if hasattr(self, 'slider_paragraph_indent_px'): self.slider_paragraph_indent_px.set(s.get('paragraph_indent_px', 40))
        if hasattr(self, 'slider_paragraph_spacing_px'): self.slider_paragraph_spacing_px.set(s.get('paragraph_spacing_px', 0))
        if hasattr(self, 'slider_toc_font_size'): self.slider_toc_font_size.set(s.get('toc_font_size', 28))
        self.slider_margin.set(s['margin'])
        self.slider_top_padding.set(s['top_padding'])
        self.slider_bottom_padding.set(s['bottom_padding'])
        self.orientation_var.set(orientation_value_to_display(s['orientation']))
        self.align_dropdown.set(align_value_to_display(s['text_align']))
        ...
        self.var_pos_title.set(pos_value_to_display(s['pos_title']))
        self.var_pos_pagenum.set(pos_value_to_display(s['pos_pagenum']))
        self.var_pos_chap_page.set(pos_value_to_display(s['pos_chap_page']))
        self.var_pos_percent.set(pos_value_to_display(s['pos_percent']))
        self.var_pos_progress.set(pos_progress_value_to_display(s['pos_progress']))
        ...
        if hasattr(self, 'var_render_antialias'): self.var_render_antialias.set(s.get("render_antialias", True))
        ...
        # 加载预设后同步所有滑块的标签与输入框（避免输入框显示旧值、需手动改回车才生效）
        for attr in dir(self):
            if attr.startswith("slider_") and not attr.endswith("_entry") and not attr.endswith("_is_float") and not attr.endswith("_label_attr"):
                if hasattr(self, attr + '_entry'):
                    self._sync_slider_ui(attr)
...
class MetadataEditorDialog(ctk.CTkToplevel):
    def __init__(self, parent, current_data, db_instance, on_save):
        ...
        self.title(trans_zh("Edit Metadata"))
        ...
        self.lbl_cover = ctk.CTkLabel(self.left_frame, text=trans_zh("Click to Change"), cursor="hand2")
        ...
        self._add_field(trans_zh("Title:"), 0, current_data[1], "entry_title")
        self._add_field(trans_zh("Author:"), 1, current_data[2], "entry_author")
        self._add_field(trans_zh("Genre:"), 2, current_data[8], "entry_genre")
        self._add_field(trans_zh("Publisher:"), 3, current_data[9], "entry_pub")
        self._add_field(trans_zh("Date:"), 4, current_data[10], "entry_date")
        ...
        ctk.CTkSwitch(tools_frame, text=trans_zh("Editor/Preview"), variable=self.var_md,
                      command=self.toggle_markdown_view, font=("Arial", 11)).pack(side="right")

        ctk.CTkLabel(self.right_frame, text=trans_zh("Description:")).grid(row=6, column=0, sticky="ne", padx=5, pady=(5, 0))
        ...
        ctk.CTkButton(btn_row, text=trans_zh("Cancel"), fg_color="gray", command=self.destroy).pack(side="left", padx=10)
        ctk.CTkButton(btn_row, text=trans_zh("Save Changes"), fg_color="green", command=self.save).pack(side="left", padx=10)
    ...
    def _render_cover(self):
        if self.current_cover_blob:
            try:
                ...
                self.lbl_cover.configure(text=trans_zh("Error loading cover"))
        else:
            self.lbl_cover.configure(image=None, text=trans_zh("No Cover\nClick to add"))
    ...
    def change_cover(self, event=None):
        ...
        if path:
            try:
                ...
                messagebox.showerror(trans_zh("Error"), trans_zh("Failed: {}").format(e))
    ...
    def save(self):
        ...
        if config.get("update_epub_cover", False) and self.epub_path and self.current_cover_blob:
            ...
            self.title(trans_zh("Injecting Cover into EPUB... Please wait"))
...
class BatchConvertDialog(ctk.CTkToplevel):
    def __init__(self, parent, selected_ids, all_books_ref, on_process_complete):
        super().__init__(parent)
        self.title(trans_zh("Batch Convert"))
        ...
        ctk.CTkLabel(self, text=trans_zh("Converting {} books").format(len(selected_ids)), font=("Arial", 16, "bold")).pack(pady=(20, 10))
        ctk.CTkLabel(self, text=trans_zh("Select Preset:")).pack(pady=(10, 5))
        ...
        self.lbl_info = ctk.CTkLabel(self, text=trans_zh("Ready to start."), text_color="gray")
        self.lbl_info.pack(pady=10)
        self.prog_bar = ctk.CTkProgressBar(self, width=300)
        self.prog_bar.set(0)
        self.prog_bar.pack(pady=15)
        self.btn_start = ctk.CTkButton(self, text=trans_zh("Start Conversion"), command=self.start_process, fg_color="#27AE60")
    ...
    def _task_batch_convert(self, settings):
        ...
        for i, bid in enumerate(self.selected_ids):
            ...
            self.update_ui(i, total, trans_zh("Converting: {}...").format(title[:20]))
        ...
        self.update_ui(total, total, trans_zh("Completed!"))
    ...
    def update_ui(self, current, total, status_msg):
        self.after(0, lambda: [
            ...
            self.btn_start.configure(text=trans_zh("Close"), state="normal", command=self.destroy) if current >= total else None
        ])
...
class FileBadge(ctk.CTkFrame):
    ...
    def _delete(self, event=None):
        if messagebox.askyesno(trans_zh("Confirm"), trans_zh("Delete this file?")): self.on_delete_click()
...
class BookDetailsWindow(ctk.CTkToplevel):
    def __init__(self, parent, book_data, on_convert, db_instance, refresh_cb):
        ...
        self.title(trans_zh("Book Details"))
    ...
    def render_ui(self):
        ...
        img_lbl = ctk.CTkLabel(cover_frame, text=trans_zh("No Cover"), width=240, height=360, fg_color="transparent",
                               corner_radius=10)
        if curr[5]:
            try:
        ...
        # Sidebar Actions
        self.btn_fetch = ctk.CTkButton(left_col, text=trans_zh("☁ Fetch Metadata"), fg_color="#8E44AD", hover_color="#9B59B6",
                                       height=40, font=("Arial", 13, "bold"), command=self.run_fetch)
        self.btn_fetch.pack(fill="x", pady=(0, 10))

        ctk.CTkButton(left_col, text=trans_zh("✎ Edit Details"), fg_color="#34495E", hover_color="#2C3E50",
        ...
        ctk.CTkButton(left_col, text=trans_zh("⚡ Open Converter"), fg_color="#D35400", hover_color="#E67E22",
                      height=50, font=("Arial", 14, "bold"),
                      command=lambda: [self.destroy(), self.on_convert_cb(curr)]).pack(fill="x", pady=(0, 10))

        ctk.CTkButton(left_col, text=trans_zh("Close"), fg_color="transparent", border_width=1, border_color="#555",
        ...
        ctk.CTkLabel(f_epub, text=trans_zh("SOURCE FILE"), font=("Arial", 10, "bold"), text_color="#666").pack(anchor="w",
                                                                                                     pady=(10, 0))

        epub_path = curr[3]
        if epub_path and os.path.exists(epub_path):
            btn = ctk.CTkButton(f_epub, text=trans_zh("📖 Open EPUB"), height=22, width=100, fg_color="transparent",
            ...
            ctk.CTkLabel(f_epub, text=trans_zh("Missing"), text_color="#C0392B", font=("Arial", 12, "bold")).pack(anchor="w")
        ...
        ctk.CTkLabel(f_xtc, text=trans_zh("CONVERTED FILE"), font=("Arial", 10, "bold"), text_color="#666").pack(anchor="w",
                                                                                                       pady=(10, 0))

        xtc_path = curr[4]
        if xtc_path and os.path.exists(xtc_path):
            btn = ctk.CTkButton(f_xtc, text=trans_zh("🖼 Open XTC"), height=22, width=100, fg_color="transparent",
                                border_width=1, border_color="#2ECC71", text_color="#2ECC71", hover_color="#333",
                                command=lambda: XtcBookViewer(self, xtc_path))
            btn.pack(anchor="w")
        else:
            ctk.CTkLabel(f_xtc, text=trans_zh("Not Converted"), text_color="gray", font=("Arial", 12)).pack(anchor="w")
        ...
        # Set to True for default Preview mode
        self.var_md = ctk.BooleanVar(value=True)
        ctk.CTkButton(frame_tools, text=trans_zh("Save Notes"), width=100, height=24, fg_color="#27AE60",
                      command=self.save_notes).pack(side="left")

        ctk.CTkSwitch(frame_tools, text=trans_zh("Editor/Preview"), variable=self.var_md,
    ...
    def run_fetch(self):
        self.btn_fetch.configure(state="disabled", text=trans_zh("☁ Fetching..."))
    ...
    def _task(self):
        ...
        if not res or res["source"] == "None":
            self.after(0, lambda: [
                self.btn_fetch.configure(state="normal", text=trans_zh("☁ Fetch Metadata")),
                messagebox.showinfo(trans_zh("Not Found"), trans_zh("No additional metadata found online."))
            ])
            return
...
class BookCard(ctk.CTkFrame, DnDWrapper):
    def __init__(self, parent, book_data, on_convert, on_details, on_delete, on_select, on_delete_file,
        ...
        self.lbl_cover = ctk.CTkLabel(self.c_f, text=trans_zh("NO COVER"), width=120, height=160, fg_color="#444")
        if book_data[5]:
            try:
        ...
        def on_status_click():
            ...
            self.btn_status.configure(text=trans_zh(new_status), fg_color=status_colors.get(new_status, "#444"))
            on_status_update(book_data[0], new_status)

        self.btn_status = ctk.CTkButton(self.c_f, text=trans_zh(status_val), width=50, height=20, corner_radius=6,
        ...
        ctk.CTkButton(act, text=trans_zh("CONVERT"), height=24, width=65, fg_color="#3498DB",
...
class BookRow(ctk.CTkFrame, DnDWrapper):
    def __init__(self, parent, book_data, on_convert, on_details, on_delete, on_select, on_delete_file,
        ...
        def on_status_click():
            ...
            self.btn_status.configure(text=trans_zh(new_s), fg_color=status_colors.get(new_s, "#444"))
            on_status_update(book_data[0], new_s)

        self.btn_status = ctk.CTkButton(self, text=trans_zh(status_val), width=70, height=24, corner_radius=6,
        ...
        ctk.CTkButton(act_frame, text=trans_zh("Convert"), width=60, height=24, fg_color="#3498DB",
                      command=lambda: on_convert(book_data)).pack(side="left", padx=2)
...
class DeviceRow(ctk.CTkFrame, DnDWrapper):
    ...
    def drag_init_out(self, event):
        ...
        self.browser.lbl_status.configure(text=trans_zh("Downloading for drag..."), text_color="#E67E22")
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                self.browser.lbl_status.configure(text=trans_zh("Ready to drag"), text_color="#2ECC71")
...
class RemoteDeviceBrowser(ctk.CTkToplevel, DnDWrapper):
    def __init__(self, parent, default_ip="192.168.0.202"):
        ...
        self.title(trans_zh("Device Manager"))
        ...
        ctk.CTkLabel(top, text=trans_zh("DEVICE IP:"), font=("Arial", 10, "bold"), text_color="gray").pack(side="left",
                                                                                                 padx=(20, 5))
        ...
        ctk.CTkButton(top, text=trans_zh("⟳ Connect"), width=100, fg_color="#34495E", command=self.refresh_root).pack(side="left",
                                                                                                            padx=10)
        ctk.CTkButton(top, text=trans_zh("⟳ Refresh"), width=100, fg_color="#34495E", command=self.refresh_current_path).pack(
            side="left", padx=10)

        self.lbl_status = ctk.CTkLabel(top, text=trans_zh("Not Connected"), font=("Arial", 12, "bold"))
        ...
        ctk.CTkButton(nav, text=trans_zh("+ New Folder"), width=100, fg_color="#27AE60", command=self.create_folder).pack(
            side="right", padx=5)

        # --- Header ---
        header = ctk.CTkFrame(self, height=30, fg_color="transparent")
        header.pack(fill="x", padx=25, pady=(0, 5))
        ctk.CTkLabel(header, text=trans_zh("NAME"), width=300, anchor="w", text_color="gray", font=("Arial", 10, "bold")).place(
            relx=0.05, rely=0)
        ctk.CTkLabel(header, text=trans_zh("SIZE"), anchor="w", text_color="gray", font=("Arial", 10, "bold")).place(relx=0.7,
                                                                                                           rely=0)
    ...
    def _fetch_list(self, path, loader):
        try:
            ...
            def _safe_loader_update(lbl, m):
                if lbl.winfo_exists():
                    lbl.configure(text=m, text_color="#E74C3C")
            self.after(0, lambda: _safe_loader_update(loader, msg))
...
class XalibreApp(ctk.CTk, DnDWrapper):
    def __init__(self):
        ...
        l1 = ctk.CTkLabel(title_box, text=trans_zh("XALIBRE"), font=("Arial", 24, "bold"), text_color="#3498DB")
        l1.pack()
        l2 = ctk.CTkLabel(title_box, text=trans_zh("MANAGER"), font=("Arial", 10, "bold"), text_color="#555")
        l2.pack(pady=(0, 2))
        ...
        self.create_icon_button(tb_top, trans_zh("＋ Import"), "#2ECC71", self.import_book_dialog)
        self.create_icon_button(tb_top, trans_zh("☑ Select All"), "#95A5A6", self.toggle_select_all)
        btn_text = trans_zh("▤ List View") if self.view_mode == "grid" else trans_zh("▦ Grid View")
        self.btn_view = self.create_icon_button(tb_top, btn_text, "#7F8C8D", self.toggle_view_mode)

        self.create_divider(tb_top)

        self.btn_conv = self.create_icon_button(tb_top, trans_zh("⚡ Convert"), "#D35400", self.open_batch_convert, "disabled")
        self.btn_fetch = self.create_icon_button(tb_top, trans_zh("☁ Fetch"), "#8E44AD", self.batch_fetch, "disabled")
        self.btn_send = self.create_icon_button(tb_top, trans_zh("📲 Send"), "#2980B9", self.open_send_dialog, "disabled")
        self.create_icon_button(tb_top, trans_zh("📟 Device Mgr"), "#34495E", self.open_device_manager)
        self.create_divider(tb_top)
        self.btn_del = self.create_icon_button(tb_top, trans_zh("🗑 Delete"), "#C0392B", self.batch_delete, "disabled")
        ...
        self.entry_search.insert(0, trans_zh("Search title or author..."))
    ...
    def handle_file_drop(self, event):
        ...
        if epubs:
            self.progress_label.configure(text=trans_zh("Importing {} books...").format(len(epubs)))
            threading.Thread(target=lambda: self._process_imports(epubs)).start()
        else:
            messagebox.showwarning(trans_zh("Invalid File"), trans_zh("Please drop .epub files only."))
    ...
    def toggle_view_mode(self):
        if self.view_mode == "grid":
            self.view_mode = "list"
            self.btn_view.configure(text=trans_zh("▦ Grid View"))
        else:
            self.view_mode = "grid"
            self.btn_view.configure(text=trans_zh("▤ List View"))
    ...
    def on_search_focus_in(self, event):
        if self.entry_search.get() == trans_zh("Search title or author..."):
    ...
    def refresh_library(self, _=None):
        ...
        if q in (trans_zh("Search title or author...").lower(), "search title or author..."): q = ""
    ...
    def batch_fetch(self):
        if not messagebox.askyesno(trans_zh("Confirm"), trans_zh("Fetch metadata for {} books?").format(len(self.selected_ids))): return
        self.btn_fetch.configure(state="disabled")
        self.progress_label.configure(text=trans_zh("Starting Metadata Fetch..."))
        self.progress_bar.set(0)
    ...
    def _task_batch_fetch(self):
        ...
        for i, bid in enumerate(list(self.selected_ids)):
            ...
            self.after(0, lambda c=i, t=total, name=safe_title: [
                ...
                self.progress_label.configure(text=trans_zh("Fetching ({}): {}").format(c + 1, name))
            ])
    ...
    def open_send_dialog(self):
        ...
        d.title(trans_zh("Send to Device"))
        d.geometry("420x480")
        d.transient(self)
        d.grab_set()
        ...
        f_fmt.pack()
        ctk.CTkCheckBox(f_fmt, text=trans_zh("Converted (.XTC)"), variable=self.var_xtc).pack(side="left", padx=10)
        ...
        ctk.CTkButton(tab_wifi, text=trans_zh("Send via Wi-Fi"), fg_color="#2980B9",
        ...
        ctk.CTkButton(tab_usb, text=trans_zh("Select Folder & Send"), fg_color="#27AE60",
